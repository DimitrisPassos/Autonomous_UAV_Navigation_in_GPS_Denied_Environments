...
class Planner:
    def __init__(self):
        ...
        self.pid_y = PID(0.1, 0.0, 0.1)
        self.pid_z = PID(0.3, 0.0, 0.1)
        self.pid_yaw = PID(0.2, 0.0, 0.2)
        self.prev_goal = [0.0] * 7
        self.min_bounds = [-33.0, -31.8, 0.5]
        self.max_bounds = [33.0, 39.0, 8.0]
        # float min_x = -3.0;
        # float max_x = 3.0;
        # float min_y = 0.0;
        # float max_y = 9.0;
        ...
        rospy.Subscriber("/best_frontier_marker", Marker, self.best_goal)
        rospy.Subscriber("/best_voxel", Point, self.bestVoxelCallback)
    ...
    def getLaserData(self, msg):
        if msg is None:
            return
        # Store the incoming laser data
        self.laser_data = msg

        # Access the ranges array (which holds distance data for each laser beam)
        ranges = msg.ranges

        # Find the minimum range, ignoring invalid values (often 0.0 or 'inf')
        valid_ranges = [r for r in ranges if r > 0]
        if valid_ranges:
            self.min_lazer = min(valid_ranges)
        

    def bestVoxelCallback(self, msg):
        if msg is None:
            return
        x = msg.x
        y = msg.y
        ...
        if x == 0 and y == 0 and z == 0:
            return
        self.best_voxel_set = point

    def set_voxel_goal(self):
        x,y,z = self.best_voxel_set
        self.set_goal(x,y,z)
    ...
    def move_to_point(self, target_x, target_y, target_z, target_yaw=None):
        ...
        while not rospy.is_shutdown():
            ...
            if np.linalg.norm([error_x, error_y, error_z]) < 1.5:
                break
...
if __name__ == '__main__':
    try:
        ...
        while not rospy.is_shutdown():
            ...
            if planner.best_voxel_set is not None:
            #planner.get_next_goal()  # Fetch a new goal
                planner.set_voxel_goal()
                planner.plan()  # Plan a path to the new goal
                planner.best_voxel_set = None
