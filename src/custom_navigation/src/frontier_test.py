...
from geometry_msgs.msg import PoseStamped, Twist, Transform, PoseWithCovarianceStamped
...
class Planner:
    def __init__(self):
        ...
        self.pid_x = PID(0.5, 0.0, 0.1)  # You can tune these gains
        self.pid_y = PID(0.5, 0.0, 0.1)
        self.pid_z = PID(0.5, 0.0, 0.1)
        self.pid_yaw = PID(0.3, 0.0, 0.2)
        self.prev_goal = [0.0] * 7
        self.min_bounds = [-50.0, -55.1, 1.0]
        self.max_bounds = [59.0, 55.0, 3.0]
        ...
        rospy.Subscriber("/large_frontier_markers", MarkerArray, self.get_goals)
        self.trajectory_pub = rospy.Publisher('/trajectory', Path, queue_size=10)
        rospy.Subscriber("/best_frontier", Marker, self.best_goal)
        rospy.Subscriber("/explore/frontiers", MarkerArray, self.get_frontiers)
        ...
        # Set bounds correctly by looping through the values
        for i in range(3):
    ...
    # def get_frontiers(self, msg):
    #     print(f"Received MarkerArray message with {len(msg.markers)} markers")

    #     # Iterate over each Marker in the MarkerArray
    #     for marker in msg.markers:
    #         # Check if the marker has points (for frontiers)
    #         if len(marker.points) == 0:
    #             print(f"Marker ID {marker.id} has no points.")
    #             continue

    #         # Loop through the points in each Marker
    #         for point in marker.points:
    #             x = point.x
    #             y = point.y
    #             self.frontier_set.append((x, y))  # Adding as a tuple to ensure uniqueness in the set

    #     print(f"Frontier points added: {self.frontier_set}")
    ...
    def get_frontiers(self, msg):
        ...
        for marker in msg.markers:
            ...
            if (self.min_bounds[0] <= x <= self.max_bounds[0] and
                self.min_bounds[1] <= y <= self.max_bounds[1]):
                dummy_set.add((x,y))
                # Adding the frontier point (x, y) to the frontier set
            self.frontier_set = dummy_set

        #print(f"Frontier points added: {self.frontier_set}")


    def set_frontier_goal(self):
        # Remove already visited nodes from frontier set
        #print("the fuck")
        self.unvisited_frontiers = [point for point in self.frontier_set if point not in self.visited_nodes]

        if len(self.unvisited_frontiers) == 0:
            print("No unvisited frontiers available")
            return

        # Get the robot's current position
        current_x, current_y = self.uav_position[0], self.uav_position[1]

        # Find the closest frontier based on Euclidean distance
        #closest_frontier = min(self.unvisited_frontiers, key=lambda point: math.sqrt((point[0] - current_x) ** 2 + (point[1] - current_y) ** 2))
        closest_frontier = random.choice(self.unvisited_frontiers)
        # Extract x, y coordinates and set the goal
        x, y = closest_frontier
        self.remove_close_frontiers(closest_frontier)
        self.visited_nodes.add((x,y))
        self.set_start()
        self.set_goal(x, y, 2)

        #print(f"Setting closest goal at: ({x}, {y})")


        
    import math

    def remove_close_frontiers(self, visited_point, proximity_threshold=5.0):
        # Iterate over the frontier set and remove points close to the visited point
        frontiers_to_remove = []
        for frontier in self.unvisited_frontiers:
            distance = math.sqrt((frontier[0] - visited_point[0])**2 + (frontier[1] - visited_point[1])**2)
            
            if distance < proximity_threshold:
                frontiers_to_remove.append(frontier)
                self.visited_nodes.add(frontier)

        # Remove the frontiers that are close to the visited point
        for frontier in frontiers_to_remove:
            self.frontier_set.remove(frontier)

        #print(f"Removed frontiers close to {visited_point}, remaining frontiers: {self.frontier_set}")



    def best_goal(self, msg):
        if msg is None:
            return
    ...
    def get_goals(self, msg):
        if msg is not None:
            for marker in msg.markers:
                ...
                # Add the new goal if it's not already in the list
                if new_goal not in self.list_of_goals and z > 0.8:
                    self.list_of_goals.append(new_goal)

            #rospy.loginfo(f"Updated list_of_goals: {self.list_of_goals}")


    # def goal_callback(self, msg):
    #     # Extract x, y, z from the PoseStamped message
    ...
    #     rospy.loginfo(f"Received goal: x = {target_x}, y = {target_y}, z = {target_z}")
        
    #     # Set the goal using the received coordinates
    #     self.set_goal(target_x, target_y, target_z, orientation_x, orientation_y, orientation_z, orientation_w)
    #     self.plan()
    ...
    def get_next_goal(self):
        if len(self.list_of_goals) == 0:
            self.set_goal(0, 0, 2)
            return
        point_random = random.choice(self.list_of_goals)
        if point_random in self.visited_nodes:
            return
        x,y,z = point_random
        #print(f"Goal selected is {x}, {y}, {z}")
        self.visited_nodes.append([x,y,z])
        self.set_goal(x,y,z)
    ...
    def add_obstacle(self, x, y, z):
        ...
        if self.should_add_collision_object():
            ...
            # Store precomputed collision object
    ...
    def is_state_valid(self, state):
        ...
        # height_state = self.uav_position[2] > 1 and self.sonar_height > 0.5
        # if height_state:
        #     return True
        #print(f"{self.objects}")
        # For the translation, you will need to directly use fcl.Transform 
        # and make sure it’s initialized correctly
        translation = fcl.Transform([x, y, z])  # Use Vector3, but check how it's handled in fcl
        ...
        for obstacle_object in collision_objects_copy:
            # obstacle_geometry = fcl.Box(1.0, 1.0, 1.0)
            # obstacle_transform = fcl.Transform([ox, oy, oz])
            # obstacle_collision_object = fcl.CollisionObject(obstacle_geometry, obstacle_transform)
    ...
    def target_is_obstacle(self, target_x, target_y, target_z, tolerance=0.5):
        ...
        for ena in collision_objects_copy:
            ...
            if ena == (target_x, target_y, target_z):
                return True
        # target = fcl.Transform([target_x, target_y, target_z])
        # uav_collision_object = fcl.CollisionObject(Box(0.80, 0.80, 0.2), target)
        # for obstacle_object in collision_objects_copy:
        #     request = fcl.CollisionRequest()
        #     result = fcl.CollisionResult()
        #     if fcl.collide(uav_collision_object, obstacle_object, request, result):
    ...
    def check_future_path_for_obstacles(self, path):
        """Check if there are obstacles in the future points of the path."""
        #current_position = self.uav_position
        num_states = path.getStateCount()
        ...
        for i in range(num_states):
            ...
            x, y, z = state.getX(), state.getY(), state.getZ()
            #print(f"Now checking for collision between {x}, {y}, {z}")
            if not self.is_state_valid(state):
                print(f"Collision detected at {state.getX()}, {state.getY()}, {state.getZ()}")
                self.stop_drone()
                return False
            #print(f"Now checking for collision between {x}, {y}, {z} and")
            # Skip points that are behind or too close to the current position
            #if np.linalg.norm([x - current_position[0], y - current_position[1], z - current_position[2]]) < 0.5:
             #   continue  # Ignore past or close points
    ...
    def plan(self, is_replan = False):
        ...
        planner.setGoalBias(0.02)  # 10% of samples will be directed toward the goal
    ...
    def navigate_smoothly(self, path):
        ...
        for i in range(num_states):
            state = path.getState(i)
            waypoints_x.append(state.getX())  # Append to the list
            waypoints_y.append(state.getY())  # Append to the list
    ...
    def execute_path(self, path):
        ...
        el_patho_x, el_patho_y, el_patho_z = self.navigate_smoothly(path)  # These are the interpolated waypoints
        rate = rospy.Rate(10)
        ...
        for i in range(len(el_patho_x)):  # Only use interpolated points
            ...
            # if self.target_is_obstacle(int(x), int(y), int(z)):
            #     rospy.loginfo("Target is an obstacle. Stopping...")
            #     self.stop_drone()
            #     return
    ...
    def rotate_to_goal(self, target_x, target_y):
        ...
        current_yaw = self.odom_rotate  # Assuming you have a function to get current yaw (orientation)
...
if __name__ == '__main__':
    try:
        ...
        # Main loop to continually fetch new goals and plan paths
        rate = rospy.Rate(0.5)  # Control the loop rate (e.g., 0.5 Hz = every 2 seconds)
        
        while not rospy.is_shutdown():
            
            #print(planner.list_of_goals)
            if len(planner.frontier_set) != 0:
            #planner.get_next_goal()  # Fetch a new goal
                planner.set_frontier_goal()
                planner.plan()  # Plan a path to the new goal
                planner.frontier_set = set()
